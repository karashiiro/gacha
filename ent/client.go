...
func NewDropClient(c config) *DropClient {
	return &DropClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `drop.Hooks(f(g(h())))`.
func (c *DropClient) Use(hooks ...Hook) {
	c.hooks.Drop = append(c.hooks.Drop, hooks...)
}

// Create returns a create builder for Drop.
func (c *DropClient) Create() *DropCreate {
	mutation := newDropMutation(c.config, OpCreate)
	return &DropCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}
...
func (c *DropClient) CreateBulk(builders ...*DropCreate) *DropCreateBulk {
	return &DropCreateBulk{config: c.config, builders: builders}
}

// Update returns an update builder for Drop.
func (c *DropClient) Update() *DropUpdate {
	mutation := newDropMutation(c.config, OpUpdate)
	return &DropUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *DropClient) UpdateOne(d *Drop) *DropUpdateOne {
	mutation := newDropMutation(c.config, OpUpdateOne, withDrop(d))
	return &DropUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *DropClient) UpdateOneID(id uint32) *DropUpdateOne {
	mutation := newDropMutation(c.config, OpUpdateOne, withDropID(id))
	return &DropUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for Drop.
func (c *DropClient) Delete() *DropDelete {
	mutation := newDropMutation(c.config, OpDelete)
	return &DropDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}
